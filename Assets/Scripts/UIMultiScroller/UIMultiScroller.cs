...
public class UIMultiScroller : MonoBehaviour, IEndDragHandler
{
    ...
    public void MoveToLine(int lineIndex, bool smooth = false)
    {
        ...
        else
        {
            ...
            {
                Debug.LogError("列表的ScrollRect的viewport没有挂载节点...");
                return;
            }
    ...
    private void CreateItem(int index)
    {
        ...
        else
        {
            if (itemPrefab == null)
            {
                Debug.LogError("itemPrefab为空！！ 请检查资源");
                return;
            }
            //GameObject obj = itemPrefab.Instantiate();
            GameObject obj = RequestGameObejct(itemPrefab);
            obj.SetActive(true);
            //obj.transform.setParentEx(_content);
    ...
    private GameObject RequestGameObejct(GameObject prefab)
    {
        ...
        if (null == go)
            go = Instantiate(prefab,_content);
