...
namespace AssetBundles
{
    ...
    public class AssetBundleLoadLevelOperation : AssetBundleLoadOperation
    {
        ...
        public AssetBundleLoadLevelOperation(string abName, string levelName, bool isAdditive, bool allowSceneActivation)
        {
        ...
        public override bool Update()
        {
            ...
            {
                if (m_IsAdditive)
                    m_Request = SceneManager.LoadSceneAsync(AssetName, LoadSceneMode.Additive);
                else
                    m_Request = SceneManager.LoadSceneAsync(AssetName, LoadSceneMode.Single);
    ...
    public class AssetBundleLoadLevelSimulationOperation : AssetBundleLoadLevelOperation
    {
        ...
        public override bool Update()
        {
            ...
            string[] levelPaths = UnityEditor.AssetDatabase.GetAssetPathsFromAssetBundleAndAssetName(AssetBundleName, AssetName);
            if (levelPaths.Length == 0)
            {
                ///@TODO: The error needs to differentiate that an asset bundle name doesn't exist
                //        from that there right scene does not exist in the asset bundle...

                m_DownloadingError = string.Format("There is no scene with name {0} in {1}", AssetName, AssetBundleName);
                return false;
            }
    ...
    public class AssetBundleLoadAssetOperationFull : AssetBundleLoadAssetOperation
    {
        ...
        public override bool Update()
        {
            ...
            // 在加载自己之前，需要确保所有的依赖项都加载完毕
            // 然后再去加载自己的项；理论上这里没有进行overlapped加载，变成了one by one的加载了！
