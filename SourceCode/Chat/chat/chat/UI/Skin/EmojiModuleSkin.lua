...
local EmojiModuleSkin = Class("EmojiModuleSkin", UISkin);
local M = EmojiModuleSkin;
...
function M:OnCreate(transform)
	...
	self.backGround = {};
	for i = 1, 3 do
		self.backGround[i] = self:GetComponentByPath("g_var/g_tab/ebtn_0" .. tostring(i) .. "/Background", "Image");
	end
